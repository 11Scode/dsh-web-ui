...
- `tests/benchmark-live-run.test.ts` pins the variant rewrites (shipped default, candidate persona, empty anchors, PTC off, official Minimal), validates every derived composition against the repository's preset schema, checks the persona block-scalar edit stops at the next composition row, reads the durable session shapes, sums usage without double counting, and fails every seed task on its initial workspace so no task passes without model work.
- `tests/benchmark-report.test.ts` pins the aggregation: Wilson bounds, the paired-by-task mean delta with its interval, infrastructure failures staying out of the denominator, and the treatment notes the report must carry.
